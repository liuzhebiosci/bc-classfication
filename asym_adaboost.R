...
dir.create(file.path(paste("/zheliu/DevGSEA/result2019/imbalanced_50_sub_", sub.ratio, sep="")), showWarnings = FALSE)
res.dir<- paste("/zheliu/DevGSEA/result2019/imbalanced_50_sub_", sub.ratio, sep="")
...
(sub.train.model$coefs)
as.factor(sub.train.class)
(sub.train.model$coef0)
abline(-cf[1]/cf[3], -cf[2]/cf[3], col = "red")



model.list[[1]]
pw.gs.anno[rep_len(seq(1, n.gs.pathway), n.iter)[order(unlist(alpha.list), decreasing = T)], 1]
order(unlist(alpha.list), decreasing = T)
alpha.list[order(unlist(alpha.list), decreasing = T)]
...
save.image(paste("/zheliu/DevGSEA/result2019/imbalanced_50_sub_", sub.ratio, "/combined weighted voting.Rdata", sep=""))
